...
type Adapter func(config.Tree) (stream.Stream, error)
...
// Register makes a stream adapter available by the provided name.
...
func Register(name string, adapter Adapter) {
	...
	if adapter == nil {
		panic("stream: Registered adapter is nil")
	}
	if _, dup := adapters[name]; dup {
		panic("stream: Duplicated adapter")
	}
...
func New(config config.Tree) (stream.Stream, error) {
	mu.RLock()
	defer mu.RUnlock()
	...
	return nil, fmt.Errorf("stream adapter not found <%s>", adapter)
}

// ErrEmptyConfig occurs when initialising stream from an empty config tree
var ErrEmptyConfig = errors.New("stream config tree is empty")
